Predictions

double(n)
I predict that this function will take a number and return double its value.
#1 square(n)
I predict this function will return the number multiplied by itself.
#2square(n)
I predict that this function will return the number multiplied by itself.
#3 isEven(n)
I predict that this function will return true if the number is even and false if it is odd.
#4 isOdd(n)
I predict that this function will return true if the number is odd and false if it is even.
#5 multiply(a, b)
I predict that this function will return the product of the two numbers.
